            ...
            <form method="POST" action="/admin/events" enctype="multipart/form-data">
              ...
              <div class="rec-box multi-day-shell ${inferredEventType === "multi-day" ? "is-visible" : ""}" id="multiDayScheduleShell" style="margin-top:14px;">
                <div style="display:flex; align-items:center; justify-content:space-between; gap:12px; margin-bottom:6px; flex-wrap:wrap;">
                  ...
                  <label class="checkbox" style="margin:0; gap:8px;">
                    <input type="checkbox" id="multiDayUseCustomDates" onchange="(function(el){if(!el||!el.checked)return;var recurringBtn=document.querySelector('[data-event-type=\"recurring\"]');var hidden=document.getElementById('eventTypeChoice');var recWrap=document.getElementById('recurrenceSettings');var hasRec=document.getElementById('hasRecurrence');var recType=document.getElementById('recurrenceType');var multi=document.getElementById('multiDayScheduleShell');if(hidden) hidden.value='recurring';if(recurringBtn&&recurringBtn.click){recurringBtn.click();}if(recWrap) recWrap.style.display='';if(hasRec) hasRec.checked=true;if(recType) recType.value='custom';if(window.ocSyncRecurrenceUI) window.ocSyncRecurrenceUI(); if(multi){multi.classList.remove('is-visible');multi.hidden=true;multi.style.display='none';} setTimeout(function(){var box=document.getElementById('customBox'); if(box&&box.scrollIntoView) box.scrollIntoView({behavior:'smooth', block:'nearest'}); el.checked=false;}, 0);})(this)" />
