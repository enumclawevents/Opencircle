...
/**
 * Preserve list state in redirects (pagination/search/archive mode)
 */
function redirectBackToAdmin(req, res) {
...
router.get("/", async (req, res) => {
  try {
    // ✅ Pagination + total count + optional server-side search + archive filter
const cols = await getEventsColumns();
const hasArchivedCols = cols.has("archived");

const limit = Math.max(10, Math.min(200, parseInt(req.query.limit || "20", 10)));
const pg = Math.max(1, parseInt(req.query.pg || "1", 10));
const offset = (pg - 1) * limit;

const q = String(req.query.q || "").trim();

// archived: "0"(active default) | "1"(archived only) | "all"
const archivedMode = String(req.query.archived || "0");
...
if (hasArchivedCols) {
  if (archivedMode === "0") whereParts.push(`archived = 0`);
  else if (archivedMode === "1") whereParts.push(`archived = 1`);
  // "all" => no filter
}
...
function adminUrl(nextPg, nextArchivedMode) {
  const sp = new URLSearchParams(req.query);
  sp.set("pg", String(nextPg));
  sp.set("limit", String(limit));
  if (q) sp.set("q", q);
  sp.set("archived", String(nextArchivedMode ?? archivedMode));
...
const isArchived = hasArchivedCols ? Number(e.archived || 0) === 1 : false;
  ...
  <div class="event-card" data-eid="${e.id}">
    ...
    <div class="event-left">
      <div class="event-title">
        ...
        ${isArchived ? `<span class="pill pill-archived" style="margin-left:8px;">Archived</span>` : ""}
      </div>
      ...
      <div class="event-actions">
        ...
        <a class="btn btn-edit" href="/admin?edit=${e.id}&pg=${pg}&limit=${limit}&archived=${encodeURIComponent(archivedMode)}${q ? `&q=${encodeURIComponent(q)}` : ""}">Edit</a>
          ...
          hasArchivedCols
            ? (
                isArchived
                  ? `
                    <form method="POST"
                          action="/admin/events/${e.id}/unarchive?pg=${pg}&limit=${limit}&archived=${encodeURIComponent(archivedMode)}${q ? `&q=${encodeURIComponent(q)}` : ""}"
                          class="inline">
                  ...
                  : `
                    <form method="POST"
                          action="/admin/events/${e.id}/archive?pg=${pg}&limit=${limit}&archived=${encodeURIComponent(archivedMode)}${q ? `&q=${encodeURIComponent(q)}` : ""}"
                          class="inline"
                          onsubmit="return confirm('Archive this event? It will be hidden from normal lists.');">
        ...
        <form method="POST"
              action="/admin/events/${e.id}/delete?pg=${pg}&limit=${limit}&archived=${encodeURIComponent(archivedMode)}${q ? `&q=${encodeURIComponent(q)}` : ""}"
              class="inline"
...
<div class="row" style="margin-top:12px;">
  ...
  <div class="actions" style="margin-top:10px;">
    <button id="addCustomDate" type="button" class="btn">+ Add Date</button>
          ...
          <div class="actions">
            ...
            ${editEvent ? `<a class="btn btn-link" href="/admin?pg=${pg}&limit=${limit}&archived=${encodeURIComponent(archivedMode)}${q ? `&q=${encodeURIComponent(q)}` : ""}">Cancel</a>` : ""}
  ...
  ${hasArchivedCols ? `
    <div class="tabs" style="margin-bottom:12px;">
      <a class="tab ${archivedMode === "0" ? "active" : ""}" href="${adminUrl(1, "0")}">Active</a>
      <a class="tab ${archivedMode === "1" ? "active" : ""}" href="${adminUrl(1, "1")}">Archived</a>
      <a class="tab ${archivedMode === "all" ? "active" : ""}" href="${adminUrl(1, "all")}">All</a>
    </div>
  ` : ``}
...
return redirectBackToAdmin(req, res);
...
router.post("/events/:id/archive", async (req, res) => {
  try {
    const cols = await getEventsColumns();
    if (!cols.has("archived")) return res.status(400).send("Archive not enabled in DB.");
